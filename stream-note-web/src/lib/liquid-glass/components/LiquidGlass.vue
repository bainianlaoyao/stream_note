...
const transformStyle = computed(() => {
  const translation = props.centered
  ...
  return `${translation} ${isActive.value && Boolean(props.onClick) ? "scale(0.96)" : calculateDirectionalScale.value}`
})
...
const positionStyles = computed<Partial<CSSProperties>>(() => {
  return {
    ...
    top: baseStyle.value.top || (props.centered ? "50%" : "0"),
    left: baseStyle.value.left || (props.centered ? "50%" : "0"),
  }
})

const isInlineMode = computed(() => props.centered === false)
</script>
