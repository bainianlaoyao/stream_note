...
import type { Vec2 } from '../shader-util'
...
interface ShaderWorkerRequest {
  width: number
  height: number
  effect: keyof typeof fragmentShaders
  mousePosition?: Vec2
  time?: number
}

// Worker message handler
self.onmessage = (e: MessageEvent<ShaderWorkerRequest>) => {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const shaderFn = (fragmentShaders[effect] || fragmentShaders.liquidGlass) as (
        uv: Vec2,
        mousePosition?: Vec2,
        time?: number,
      ) => Vec2
      const pos = shaderFn(uv, mousePosition, time)
      const dx = pos.x * w - x
      const dy = pos.y * h - y
